...
def build_model(n_hidden=1, n_neurons=30, input_shape=[X_train_std.shape[1]], rate=0.1):
...
start = time()
nn.fit(X_train_std, y_train_le, epochs=50,
...
model = keras.models.Sequential([
    ...
    keras.layers.Dense(nn.best_params_["n_neurons"], activation="selu", kernel_initializer="lecun_normal"),
    keras.layers.AlphaDropout(rate=nn.best_params_["rate"]),
    keras.layers.Dense(nn.best_params_["n_neurons"], activation="selu", kernel_initializer="lecun_normal"),
    keras.layers.AlphaDropout(rate=nn.best_params_["rate"]),
    keras.layers.Dense(nn.best_params_["n_neurons"], activation="selu", kernel_initializer="lecun_normal"),
    keras.layers.AlphaDropout(rate=nn.best_params_["rate"]),
...
# fit model
history = model.fit(X_train, y_train_le, epochs=50, validation_split=0.1, batch_size=500,
                    callbacks=[keras.callbacks.EarlyStopping(patience=10, restore_best_weights=True)])
...
y_train_probas = model.predict(X_train_std)[:, 1]

thresholds = np.linspace(0.05, .07, 11)
...
optimal = thresholds[cv_results.argmax()] # 0.064

# evaluate model on val set
y_val_probas = model.predict(X_val_std)
...
# monte carlo dropout
y_train_probas = np.stack([model(X_train_std, training=True) for sample in range(100)])
y_train_probas = y_train_probas.mean(axis=0)
y_train_probas = y_train_probas[:,1]
# find best threshold value
thresholds = np.linspace(0.33, .35, 11)
...
cv_results = np.mean(balanced_acc_cv, axis=0)
plt.plot(thresholds, cv_results)
optimal = thresholds[cv_results.argmax()] # 0.342


y_val_probas = np.stack([model(X_val_std, training=True) for sample in range(100)])
